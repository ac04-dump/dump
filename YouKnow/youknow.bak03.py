...
if __name__ == "__main__":
    ...
    while pos < dur:
        ...
        if max_per_image > 7:
            ...
            see = max_per_image
        img_args += f" -i '{images[i]}' -filter_complex \"[{clip}:v] scale={width}x{height} [ovrl{i}]; [0:v][ovrl{i}] overlay=x=(main_w-overlay_w)/2:y=(main_h-overlay_h)/2:enable='between(t,{pos},{pos+see})'\""
        pos += see
        i += 1
        clip += 1
