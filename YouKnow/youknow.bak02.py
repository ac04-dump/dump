...
if __name__ == "__main__":
    ...
    while pos < dur:
        ...
        if max_per_image > 3:
            ...
            see = max_per_image
        img_args += f" -i '{images[i]}' -filter_complex \"[1:v] scale={width}x{height} [ovrl]; [0:v][ovrl] overlay=x=(main_w-overlay_w)/2:y=(main_h-overlay_h)/2:enable='between(t,{pos},{pos+see})'\""
        pos += see
        i += 1

    command = f"ffmpeg -hide_banner" + \
            " -loop 1 -i /home/alex/bg.png" + \
            img_args + \
            f" -i {TEMP_SPEECH_FILE} -filter:a atempo=1.3" + \
            f" -shortest -vcodec mpeg4 {OUTPUT_FILE}"
    print(command)
    os.system(command)
    os.system(f"mpv {OUTPUT_FILE}")

            #f" -i {images[0]} -filter_complex '[1:v]scale=800:800 [ovrl], [0:v][ovrl]overlay=x=(main_w-overlay_w)/2:y=(main_h-overlay_h)/2'"
